...
from .GSIEA_tools import MultiModalEncoder
from .GSIEA_loss import CustomMultiLossLayer,icl_loss

from src.utils import pairwise_distances

class GSIEA(nn.Module):
    def __init__(self,kgs,args):
        ...
        self.multimodal_encoder = MultiModalEncoder(args=self.args,ent_num=kgs["ent_num"],img_feature_dim=img_dim,char_feature_dim=char_dim,use_project_head=self.args.use_project_head,attr_input_dim=kgs["att_features"].shape[1])

        self.multi_loss_layer = CustomMultiLossLayer(loss_num=5)
    ...
    def forward(self,batch):
        ...
        in_loss = self.inner_view_loss(gph_emb, rel_emb, att_emb, name_emb, char_emb, batch,weight_norm=weight_norm)
        out_loss = self.inner_view_loss(gph_emb_hid, rel_emb_hid, att_emb_hid, name_emb_hid, char_emb_hid,batch)
    ...
    def inner_view_loss(self,gph_emb,rel_emb,att_emb,img_emb,name_emb,char_emb,train_ill,weight_norm=None):
        if weight_norm is not None:
            ...
            loss_GCN = self.criterion_cl(gph_emb, train_ill, weight_norm=weight_norm[:, 0]) if gph_emb is not None else 0
            loss_rel = self.criterion_cl(rel_emb, train_ill,weight_norm=weight_norm[:, 1]) if rel_emb is not None else 0
            loss_att = self.criterion_cl(att_emb, train_ill,weight_norm=weight_norm[:, 2]) if att_emb is not None else 0
            loss_img = self.criterion_cl(img_emb, train_ill,weight_norm=weight_norm[:, 3]) if img_emb is not None else 0
    ...
    def joint_emb_generat(self,only_joint=True):
        gph_emb,img_emb,rel_emb,att_emb,\
            name_emb,char_emb,joint_emb,joint_emb_fz,hidden_states,weight_norm = self.multimodal_encoder(self.input_idx,self.adj,self.img_features,self.rel_features,self.att_features,self.name_features,self.char_features)
        if only_joint:
