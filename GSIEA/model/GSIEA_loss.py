...
class icl_loss(nn.Module):
    ...
    def softXEnt(self, target, logits, replay=False, neg_cross_kg=False,w_min=None):
    ...
    def forward(self, emb, train_links, neg_l=None, neg_r=None, weight_norm=None,norm=True):
        ...
        if self.replay:
            loss_a, a_neg_idx = self.softXEnt(labels, logits_a, replay=True, neg_cross_kg=self.neg_cross_kg,w_min=score_w_min)
            if neg_l is not None:
                loss_b, b_neg_idx = self.softXEnt(labels_2, logits_b, replay=True, neg_cross_kg=self.neg_cross_kg,w_min=score_w_min)
                #
                a_ea_cand = torch.cat([train_links[:, 1], train_links[:, 0], neg_l]).cuda()
                b_ea_cand = torch.cat([train_links[:, 0], train_links[:, 1], neg_r]).cuda()
            else:
                loss_b, b_neg_idx = self.softXEnt(labels, logits_b, replay=True, neg_cross_kg=self.neg_cross_kg,w_min=score_w_min)
