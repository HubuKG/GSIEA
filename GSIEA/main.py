...
class Runner:
    ...
    def model_choise(self):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "GSIEA"]
        if self.args.model_name == "GSIEA":
            self.model = GSIEA(self.KGs, self.args)
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["GSIEA"]:
